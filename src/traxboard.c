 ...
 *
 * [] Last Modified : Thu 16 Apr 2015 05:50:54 PM IRDT
 *
...
static const int EMPTY = 0, INVALID = 7,
	NS = 1, SN = 1, WE = 2, EW = 2, NW = 3, WN = 3, NE = 4, EN = 4, WS = 5,
	SW = 5, SE = 6, ES = 6;
static const int WHITE = 0, BLACK = 1, DRAW = 2, NOPLAYER = 3, NORESULT = 3;
...
void makeMove(char *move)
{
	...
	switch (neighbor) {
		...
		case 18:
			switch (direction) {
				...
				case 'D':
					throw
					new IllegalMoveException("illegal direction.");
				default:
					/* This should never happen */
					throw
		...
		case 36:
			if (direction == '/') putAt(row, col, NW);
	                ...
	                if (direction == 'L')
		                throw
			                new
		                IllegalMoveException(move + " illegal direction.");
	                if (direction == 'R') putAt(row, col, WN);
	                if (direction == 'U') putAt(row, col, WN);
	                if (direction == 'D') throw
		                new IllegalMoveException("illegal direction.");
	                break;
		case 40:
			if (direction == '/') throw
		...
		case 64:
			if (direction == '/') putAt(row, col, ES);
	                ...
	                if (direction == 'C') throw
		                new IllegalMoveException("illegal direction.");
	                if (direction == 'L') throw
		                new IllegalMoveException("illegal direction.");
	                if (direction == 'R') throw
		                new IllegalMoveException("illegal direction.");
	...
	if (!forcedMove(row - 1, col)) {
		restoreState();
		throw
		new IllegalMoveException("illegal filled cave.");
	}
	if (!forcedMove(row + 1, col)) {
		restoreState();
		throw
...
ArrayList <String> Moves = new
ArrayList <>(100); // 50 might be enough
String AMove;
int i, j, k;
int dl, dr, ur, ul, rr, dd;
int[][]
neighbors = new
int[10][10]; // which neighbors - default all
// values 0
boolean[][][]
dirlist = newboolean[10][10][3]; // which directions for
// move
// 0 /, 1 \, 2 +
...
/* This should never happen */
throw new
RuntimeException("This should never happen.");
}
Moves.
...
private booleancheckLine(int row, int col, char direction, char type)
{
	return checkLine(0, 0, false, row, col, direction, type);
}

private boolean
	checkLine(int searchTileRow, int searchTileCol, boolean winner, int row, int col, char direction, char type)
{
...
private booleanisLeftRightMirror()
{
	int piece, i, j, j2;
...
private booleanisRightLeftMirror()
{
	return isLeftRightMirror();
}

private booleanisUpDownMirror()
{
	int piece, i, j, i2;
...
private booleanisDownUpMirror()
{
	return isUpDownMirror();
}

// 90 degree rotation
private booleanisRotateMirror()
{
	int i, j, piece, i2, j2;
...
public booleanwinnerTile(int row, int col)
{
...
private booleancanMoveDown()
{
	return (getRowSize() < 8);
}

private booleancanMoveRight()
{
	return (getColSize() < 8);
}

private booleanforcedMove(int brow, int bcol)
{
...
int[][]
wnum = new
int[9][9]; // every white line gets a number
int[][]
bnum = new
int[9][9];
int currentWNum = 1;
...
private booleancheckThreatPosition(int row, int col, char direction, int west, int east, int north, int south)
{
	...
	if (DEBUG) {
		...
		System.err.println(
			"checkThreatPosition:west=" + west + ", east=" + east + ", north=" + north + ", south=" +
			south);
	}
	switch (direction) {
		case 'N':
...
private void checkThreat(int row, int col, char direction, String threat)
{
	if (DEBUG)
		System.err.println(
			"checkThreat: row=" + row + ", col=" + col + ", direction=" + direction + ", threat=" + threat);
	switch (threat) {
		case "W+W-BB-W+W":
		case "W+W-BBB-W+W":
	                ...
	                switch (direction) {
		                ...
		                case 'S':
			                if (getAt(row, col) == Traxboard.WE) {
				                whiteCorners++;
				                if (checkThreatPosition(row, col, direction, 6, 1, 0,
					                1)) { whiteThreats++; }
			                }
	                                break;
		                case 'E':
			...
			if (!checkThreatPosition(row, col, direction, 0, 1, 0, 1)) { break; }
	                switch (direction) {
		                ...
		                case 'S':
			                if (getAt(row, col) == Traxboard.WE) {
				                whiteCorners++;
				                if (checkThreatPosition(row, col, direction, 7, 1, 0,
					                1)) { whiteThreats++; }
			                }
	                                break;
		                case 'E':
			...
			if (!checkThreatPosition(row, col, direction, 0, 1, 0, 1)) break;
	                switch (direction) {
		                ...
		                case 'S':
			                if (getAt(row, col) == Traxboard.WE) {
				                whiteCorners++;
				                if (checkThreatPosition(row, col, direction, 6, 1, 0, 1))
					                whiteThreats++;
			                }
	                                break;
		                case 'E':
			...
			if (!checkThreatPosition(row, col, direction, 0, 1, 0, 1)) break;
	                switch (direction) {
		                ...
		                case 'S':
			                if (getAt(row, col) == Traxboard.WE) {
				                whiteCorners++;
				                if (checkThreatPosition(row, col, direction, 5, 1, 0, 1))
					                whiteThreats++;
			                }
	                                break;
		                case 'E':
			...
			if (!checkThreatPosition(row, col, direction, 0, 1, 0, 0)) break;
	                switch (direction) {
		                case 'E':
			                if (getAt(row, col) == Traxboard.WE) {
				                blackCorners++;
				                if (getAt(row - 4, col - 1) == Traxboard.WE &&
				                    checkThreatPosition(row, col, direction, 0, 0, 0,
					                    2)) { blackThreats++; }
			                }
	                                break;
		                case 'W':
		                ...
		                case 'N':
			                if (getAt(row, col) == Traxboard.NS) {
				                blackCorners++;
				                if (getAt(row + 1, col - 4) == Traxboard.NS &&
				                    checkThreatPosition(row, col, direction, 0, 0, 0,
					                    2)) { blackThreats++; }
			                }
	                                break;
			...
			if (!checkThreatPosition(row, col, direction, 0, 1, 0, 0)) break;
	                switch (direction) {
		                case 'E':
			                if (getAt(row, col) == Traxboard.NS) {
				                whiteCorners++;
				                if (getAt(row - 4, col - 1) == Traxboard.NS &&
				                    checkThreatPosition(row, col, direction, 0, 0, 0,
					                    2)) { whiteThreats++; }
			                }
	                                break;
		                ...
		                case 'N':
			                if (getAt(row, col) == Traxboard.WE) {
				                whiteCorners++;
				                if (getAt(row + 1, col - 4) == Traxboard.WE &&
				                    checkThreatPosition(row, col, direction, 0, 0, 0,
					                    2)) { whiteThreats++; }
			                }
	                                break;
			...
			if (!checkThreatPosition(row, col, direction, 0, 1, 0, 0)) break;
	                switch (direction) {
		                case 'S':
			                if (getAt(row, col) == Traxboard.WE) {
				                whiteCorners++;
				                if (getAt(row - 1, col + 3) == Traxboard.WE &&
				                    checkThreatPosition(row, col, direction, 0, 0, 0,
					                    2)) { whiteThreats++; }
			                }
	                                break;
		                ...
		                case 'W':
			                if (getAt(row, col) == Traxboard.NS) {
				                whiteCorners++;
				                if (getAt(row + 3, col + 1) == Traxboard.NS &&
				                    checkThreatPosition(row, col, direction, 0, 0, 0,
					                    2)) { whiteThreats++; }
			                }
	                                break;
			...
			if (!checkThreatPosition(row, col, direction, 0, 1, 0, 0)) break;
	                switch (direction) {
		                case 'E':
			                if (getAt(row, col) == Traxboard.WE) {
				                blackCorners++;
				                if (getAt(row - 3, col - 1) == Traxboard.WE &&
				                    checkThreatPosition(row, col, direction, 0, 0, 0,
					                    2)) { blackThreats++; }
			                }
	                                break;
		                ...
		                case 'S':
			                if (getAt(row, col) == Traxboard.NS) {
				                blackCorners++;
				                if (getAt(row - 1, col + 3) == Traxboard.NS &&
				                    checkThreatPosition(row, col, direction, 0, 0, 0,
					                    2)) { blackThreats++; }
			                }
	                                break;
		...
		default:
			// This should never happen
			throw
			new RuntimeException("This should never happen. [" + threat + "]");
	}
}
...
public static Traxboard
changeColors(Traxboard
tb) {
return
changeColours(tb);
}
...
public static Traxboard
changeColours(Traxboard
tb)
{
Traxboard result = newTraxboard(tb);
for (
int i = result.firstrow;
i<=result.
lastrow;
i++) {
for (
int j = result.firstcol;
j<=result.
lastcol;
j++) {
...
WE:
	result
.board[i][tb.lastcol-j+tb.firstcol]=tb.board[i][j];
break;
case
NW:
	result
.board[i][tb.lastcol-j+tb.firstcol]=
NE;
break;
case
NE:
